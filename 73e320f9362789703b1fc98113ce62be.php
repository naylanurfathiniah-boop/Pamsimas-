...
<head>
    ...
    <title><?php echo $__env->yieldContent('title','Dashboard'); ?> — PAMSIMAS</title>
...
<aside id="sidebar"
   ...
   class="fixed top-0 left-0 h-screen bg-white dark:bg-gray-900 border-r border-gray-100 dark:border-gray-800 z-40 flex flex-col shadow-2xl lg:shadow-none overflow-hidden" style="position:fixed;top:0;left:0;height:100vh;"
    ...
    <div x-show="!sidebarCollapsed" class="px-4 py-3 border-b border-gray-100 dark:border-gray-800">
        <div class="flex items-center gap-2.5 p-2.5 rounded-xl bg-brand-50 dark:bg-brand-900/30">
            ...
            <div class="overflow-hidden">
                <p class="text-gray-800 dark:text-gray-200 text-sm font-semibold truncate"><?php echo e(auth()->user()->name); ?></p>
                <p class="text-brand-600 dark:text-brand-400 text-xs capitalize"><?php echo e(auth()->user()->role); ?></p>
            </div>
        </div>
    </div>
    ...
    <nav class="flex-1 overflow-y-auto py-3 px-2 space-y-0.5 scroll-smooth">
        ...
        <?php $__currentLoopData = $adminSections; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $section): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <?php if($section['group']): ?>
            <p x-show="!sidebarCollapsed" class="px-3 pt-3 pb-1 text-xs font-semibold text-gray-400 uppercase tracking-wider"><?php echo e($section['group']); ?></p>
            <?php endif; ?>
            ...
            ?>
            <a href="<?php echo e(route($item['r'])); ?>"
                @click="if(window.innerWidth < 1024) sidebarOpen = false"
                class="sidebar-link <?php echo e($isActive ? 'active' : ''); ?> flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all
                <?php echo e($isActive ? 'bg-brand-50 dark:bg-brand-900/40 text-brand-700 dark:text-brand-300' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-white'); ?>">
                <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="<?php echo e($item['i']); ?>"/>
                </svg>
                <span x-show="!sidebarCollapsed" class="flex-1 truncate"><?php echo e($item['l']); ?></span>
                <?php if($badgeCount > 0): ?>
                <span x-show="!sidebarCollapsed"
                    class="flex-shrink-0 w-5 h-5 <?php echo e($item['badge']==='notif' ? 'bg-red-500' : 'bg-amber-500'); ?> text-white text-xs rounded-full flex items-center justify-center font-bold">
                    <?php echo e(min($badgeCount, 9)); ?>
        ...
        <?php $__currentLoopData = $petugasLinks; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <a href="<?php echo e(route($item['r'])); ?>"
            @click="if(window.innerWidth < 1024) sidebarOpen = false"
            class="sidebar-link <?php echo e(request()->routeIs($item['r'].'*') ? 'active' : ''); ?> flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all
            <?php echo e(request()->routeIs($item['r'].'*') ? 'bg-brand-50 dark:bg-brand-900/40 text-brand-700 dark:text-brand-300' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-white'); ?>">
            <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="<?php echo e($item['i']); ?>"/>
            </svg>
            <span x-show="!sidebarCollapsed"><?php echo e($item['l']); ?></span>
        </a>
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        <?php endif; ?>
        ...
        <?php $__currentLoopData = $pelangganLinks; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <a href="<?php echo e(route($item['r'])); ?>"
            @click="if(window.innerWidth < 1024) sidebarOpen = false"
            class="sidebar-link <?php echo e(request()->routeIs($item['r'].'*') ? 'active' : ''); ?> flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all
            <?php echo e(request()->routeIs($item['r'].'*') ? 'bg-brand-50 dark:bg-brand-900/40 text-brand-700 dark:text-brand-300' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-white'); ?>">
...
<div :class="{ 'lg:ml-72': !sidebarCollapsed, 'lg:ml-16': sidebarCollapsed }"
    ...
    <header class="sticky top-0 z-20 bg-white/95 dark:bg-gray-900/95 backdrop-blur border-b border-gray-100 dark:border-gray-800 h-16 flex items-center px-4 sm:px-6 gap-3 shadow-sm">
        ...
        <div class="flex-1 min-w-0">
            <h1 class="text-base font-bold text-gray-800 dark:text-white truncate"><?php echo $__env->yieldContent('page_title','Dashboard'); ?></h1>
            <p class="text-gray-400 text-xs hidden sm:block truncate"><?php echo $__env->yieldContent('page_subtitle',''); ?></p>
        </div>
        ...
        <div class="flex items-center gap-1 flex-shrink-0">
            ...
            <?php $notifCount = auth()->user()->notifBelumDibaca(); ?>
                ...
                <div x-show="open" @click.outside="open = false"
                    ...
                    <div class="px-4 py-3 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
                        <div class="flex items-center gap-2">
                            ...
                            <?php if($notifCount > 0): ?>
                            <span class="px-2 py-0.5 bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-300 text-xs rounded-full font-semibold"><?php echo e($notifCount); ?></span>
                            <?php endif; ?>
                        </div>
                        <div class="flex items-center gap-3">
                            ...
                            <?php endif; ?>
                            <?php if(auth()->user()->role === 'admin'): ?>
                            <a href="<?php echo e(route('admin.notifikasi.index')); ?>" @click="open=false" class="text-xs text-gray-400 hover:text-brand-600">Semua →</a>
                            <?php endif; ?>
                        </div>
                    </div>

                    <div class="max-h-72 overflow-y-auto divide-y divide-gray-50 dark:divide-gray-800">
                        <?php $notifList = auth()->user()->notifikasi()->latest()->take(6)->get(); ?>
                        ...
                        <div class="flex items-start gap-3 px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors <?php echo e(!$n->sudah_dibaca ? 'bg-brand-50/40 dark:bg-brand-900/10' : ''); ?>">
                            <div class="w-8 h-8 rounded-lg flex-shrink-0 flex items-center justify-center mt-0.5
                                ...
                                <?php if($n->tipe==='success'): ?><svg class="w-4 h-4 text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                                <?php elseif($n->tipe==='warning'): ?><svg class="w-4 h-4 text-amber-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>
                                <?php elseif($n->tipe==='error'): ?><svg class="w-4 h-4 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                                <?php else: ?><svg class="w-4 h-4 text-brand-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg><?php endif; ?>
                            </div>
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-semibold text-gray-800 dark:text-gray-200 leading-tight">
                                    <?php echo e($n->judul); ?>

                                    <?php if(!$n->sudah_dibaca): ?><span class="inline-block w-1.5 h-1.5 rounded-full bg-brand-500 ml-1 align-middle"></span><?php endif; ?>
                                </p>
                                <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5 line-clamp-2"><?php echo e($n->pesan); ?></p>
                                <p class="text-xs text-gray-300 dark:text-gray-600 mt-1"><?php echo e($n->created_at->diffForHumans()); ?></p>
                            </div>
                        </div>
            ...
            <div class="relative" x-data="{ open: false }">
                <button @click="open = !open"
                    ...
                    <div class="hidden sm:block text-left">
                        <p class="text-sm font-semibold text-gray-800 dark:text-white leading-tight"><?php echo e(Str::limit(auth()->user()->name, 12)); ?></p>
                ...
                <div x-show="open" @click.outside="open = false"
                    ...
                    <div class="p-3 border-b border-gray-100 dark:border-gray-800">
                        ...
                        <span class="inline-block mt-1.5 px-2 py-0.5 rounded-full text-xs font-semibold capitalize
                            <?php echo e(auth()->user()->role==='admin' ? 'bg-purple-100 text-purple-700 dark:bg-purple-900/40 dark:text-purple-300'
    ...
    <footer class="border-t border-gray-100 dark:border-gray-800 px-6 py-3 flex items-center justify-between">
        <p class="text-xs text-gray-400">© <?php echo e(date('Y')); ?> PAMSIMAS — Sistem Air Minum Berbasis Masyarakat</p>
...
<?php /**PATH C:\laragon\www\PamsimasBumnag\resources\views/layouts/app.blade.php ENDPATH**/ ?>